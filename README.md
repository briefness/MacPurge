...
| 应用卸载 | 读取 `/Applications` 与 `~/Applications`，按 Bundle ID 识别已确认的应用残留 |
...
- 应用仍在运行或扫描结果不完整时，应用本体及其所有关联候选都会被锁定，必须先退出应用或重新扫描。
...
个人在自己的 Mac 上运行不需要 Apple Developer Program、Developer ID 或公证。本项目不接受对外分发、商业销售或集成到付费产品。
...
`cleanmymac-cli` 当前使用 MacPaw 的终端用户许可协议（EULA），不是 OSI 认可的开源许可证。该协议将使用权限定为个人、非商业用途，并限制未经书面同意的复制、修改、衍生、分发和商业使用。请阅读上游的 [LICENSE](https://github.com/MacPaw/cleanmymac-cli/blob/main/LICENSE) 原文。

本仓库不包含上游源码；为避免许可边界和来源混淆，本项目同样明确限定为个人使用与研究，不得商用。若要改变这一范围，应先取得相关权利人的书面许可并重新完成许可证审查。
...
本仓库当前未授予开源许可证，也不授予商业使用、再分发、再许可或对外销售权。未经项目维护者明确书面授权，请仅用于个人学习、测试和研究。
